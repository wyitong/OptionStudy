...
def futureIopvOLS(data):
    X1 = data['futureR'].values - 1
    X1= sm.add_constant(X1)
    y1 = data['iopvR'].values - 1
...
def fut_iopv_compare(data):
    ...
    y1 = data['optionSpot'].values
    model1 = sm.OLS(y1,X1,missing='drop').fit()
    model2= sm.OLS(y1,X2,missing='drop').fit()

    print("Date: "+dt.datetime.strftime(data['date_x'].iloc[0], format="%Y%m%d")+"FutR2 {:.2%}, IopvR2 {:.2%}".format(model1.rsquared, model2.rsquared))
...
def gen_iopv_spottheory_ols(olsWindow, mergedDF):
    ...
    for row in mergedDF.itertuples():
        if(not np.isnan(row.iopvR)):
            fut_r_array.append(row.futureR-1)
            iopv_r_array.append(row.iopvR-1)
    ...
    mergedDF['iopvR'+str(ols_windowSize)] = mergedDF['b0'] + mergedDF['b1'] * (mergedDF['futureR'] -1)+1
...
def gen_iopv_spottheory_bo(BoWindow, mergedDF, iopvBeta = 0.98):
    """   
    ...
    validDF['rDiff'] = (validDF['iopvR'] -1) - (validDF['futureR']-1) * slope
    ...
    mergedDF2['iopvR'+str(futureBoWindow)+'bo'] = ((mergedDF2['futureR'] -1) * slope+1) + mergedDF2['rdiff_ma'+str(futureBoWindow)]
...
if __name__ == '__main__':
    ...
    # %%
    futureIopvOLS(mergedDF)
    # %%
    fut_iopv_compare(mergedDF)
    ...
    # %%
    #OLS to explain iopv from future
    os.chdir("C:/Users/Yitong/AppData/Local/auto-option-mm/SpotDetail")
    ...
    for ols_window in ols_windowlist:
        ...
        plt.show()
    # %%
    #fixed slope to explain iopv from future
    ols_windowlist = [12,25,50,100,150,200,300]
    for ols_window in ols_windowlist:
        botheoR2 = []
        iopvtheoR2 = []
