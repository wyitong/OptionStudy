...
# paramlist = [(0.001,0.00025,0.004,4000, 0.4)]
paramlist = [(0.001,0.00025,0.005,4000, 0.9)]
...
paramlist = [(0.001,0.00025,0.001,4000)]
paramlist.append((0.001,0.00025,0.004,4000))
paramlist.append((0.001,0.00025,0.005,4000))
paramlist.append((0.001,0.00025,0.006,4000))
paramlist.append((0.001,0.00025,0.007,4000))
paramlist.append((0.001,0.00025,0.0085,4000))
paramlist.append((0.001,0.00025,0.01,4000))
paramlist.append((0.002,0.00025,0.002,4000))
paramlist.append((0.002,0.00025,0.003,2000))
paramlist.append((0.002,0.00025,0.003,4000))
paramlist.append((0.002,0.00025,0.003,10000))
paramlist.append((0.002,0.00025,0.0035,4000))
paramlist.append((0.002,0.00025,0.004,4000))
paramlist.append((0.002,0.00025,0.0045,4000))
paramlist.append((0.002,0.00025,0.005,4000))
paramlist.append((0.002,0.00025,0.006,4000))
#%%
...
def produceBtSum(plcostDF,expstr,paramlist):
    ...
    resDict.update({'openThreshold':paramlist[0],'posDiff':paramlist[1],'closeOffest':paramlist[2],'posVega':paramlist[3]})
...
for param in paramlist:
    ...
    plcost03 = LoadCostPnl("PnlCost_20200325_20200220.csv","03")
    reslist.append(produceBtSum(plcost03,'03',param))
    plcost06 = LoadCostPnl("PnlCost_20200624_20200220.csv","06")
    reslist.append(produceBtSum(plcost06,'06',param))
    plcost09 = LoadCostPnl("PnlCost_20200923_20200220.csv","09")
    ...
    allPlCost['TotalCost'] = allPlCost['TotalCost01'] + allPlCost['TotalCost02'] + allPlCost['TotalCost03'] + allPlCost['TotalCost06'] + allPlCost['TotalCost09']
    allPlCost['TotalPnL'] = allPlCost['TotalPnL01'] + allPlCost['TotalPnL02'] + allPlCost['TotalPnL03'] + allPlCost['TotalPnL06'] + allPlCost['TotalPnL09']
...
sumDF = sumDF[['Expiry', 'openThreshold', 'closeOffest', 'posDiff','posVega','Vega/VolPoint','PnL','Cost','Return','AReturn','PnL_pre','Cost_pre','Return_pre','AReturn_pre','PnL_exc','Cost_exc','Return_exc','AReturn_exc']]
os.chdir(rootpath+"/Summary/")
sumDF.to_excel("Summary_Auto_v0.xlsx")
...
for param in paramlist:
    ...
    trades03 = LoadVSTrades("ButterKnifeZeroTrades_20200325_20200220.csv","03")
    trades06 = LoadVSTrades("ButterKnifeZeroTrades_20200624_20200220.csv","06")
    trades09 = LoadVSTrades("ButterKnifeZeroTrades_20200923_20200220.csv","09")
    allTrades =  trades01.append([trades02,trades03,trades06,trades09])
...
sumDF = pd.DataFrame(reslist)
sumDF['Vega/VolPoint'] = sumDF['posVega'] /sumDF['posDiff']/100
sumDF = sumDF[['Expiry', 'openThreshold', 'closeOffest', 'posDiff','posVega','Vega/VolPoint','Count','LongSH','HoldingMin','NetPnl','TotalPnlTheory','PnlCompleteRatio','Leg1Pnl','Leg2Pnl','SHVegaCompletionRate','SZVegaCompletionRate','SHDeltaCompletionRate','SZDeltaCompletionRate','OpenExecuMin','CloseExecuMin']]
os.chdir(rootpath+"/Summary/")
sumDF.to_excel("Summary_Trades_v0.xlsx")
