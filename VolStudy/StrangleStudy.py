...
os.chdir("C:/Users/Yitong/AppData/Local/auto-option-mm/StranglePrice/")
strangle_DF = pd.DataFrame()
for filename in os.listdir(os.getcwd()):
    if filename.startswith("Strangle"):
        ...
        if filedate<=end_date and filedate>=starting_date and expdate ==studyexp:
            print(filename)
            strangle_DF = strangle_DF.append(pd.read_csv(filename))

print(np.std(strangle_DF['YDP30']))
print(np.std(strangle_DF['YDP60']))
print(np.std(strangle_DF['YDP120']))
print(np.std(strangle_DF['YDP300']))
print(np.std(strangle_DF['YDP3600']))
